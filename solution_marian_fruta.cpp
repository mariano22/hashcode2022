...
struct SingleAnswer {
    ...
    int p_idx;
    double ratio;
    SingleAnswer(int p_idx) : p_idx(p_idx) {
        fail = true;
        ratio = 0;
        t_start = 1e9;
        t_end = 0;
...
SingleAnswer assign_with_t(int p_idx,int p_start_time) {
    int R =si(p_reqs[p_idx]); 
    SingleAnswer result(p_idx);
    ...
    forn(r,R) {
        ...
        forn(c_idx, C) if (!c_selecteds_set.count(c_idx) && c_busy[c_idx] <= p_start_time) {
            ...
            auto jt = s_selected_set.find(s_idx);
            if ( ( c_lvl >= req_lvl || (jt!=s_selected_set.end() && jt->snd>=req_lvl && c_lvl==req_lvl-1)) && 
        ...
        if (c_selected==-1) {
            ...
            for(const auto& [other_skill_id, other_skill_lvl] : c_skill[c_selected]) {
                auto jt = s_selected_set.find(other_skill_id);
                if (jt!=s_selected_set.end()) {
                    jt->snd = max(jt->snd, other_skill_lvl);
                } else {
                    s_selected_set[other_skill_id]=other_skill_lvl;
                }
            }
        }
    }
...
#define EPS 1e-8
SingleAnswer assign_pidx(int p_idx) {
    int R =si(p_reqs[p_idx]); 
    int a = 0;
    //~ int b = 2e5;
    ...
    while(b-a > 20) {
        int l = (2*a+b) / 3;
        int r = (a+2*b) / 3;
        SingleAnswer l_ans = assign_with_t(p_idx, l);
        SingleAnswer r_ans = assign_with_t(p_idx, r);
        if ( r_ans.fail ) {
            a = r+1;
        } else if ( l_ans.fail ) {
            a = l+1;
    ...
    SingleAnswer ans(p_idx);
    forsn(p_start_time, a,b+1) {
        SingleAnswer curr = assign_with_t(p_idx, p_start_time);
...
ll solve() {
    anss.clear();
    forn(f,2) p_ord[f].resize(P);
    forn(i,P) {
        forn(f,2) p_ord[f][i]=i;
    }
    sort(p_ord[0].begin(), p_ord[0].end(), [] (const int& i, const int& j) {
    ...
    sort(p_ord[1].begin(), p_ord[1].end(), [] (const int& i, const int& j) {
        //~ return mp(p_value[i],i)>mp(p_value[j],j);
        //~ return mp(p_before[i],i)<mp(p_before[j],j);
        return mp(p_before[i]-p_takes[i],i)<mp(p_before[j]-p_takes[j],j);
        //~ return mp(p_score[i],i)>mp(p_score[j],j);
    ...
    forn(p_idx,P) {
        ...
        sort(r_ord[p_idx].begin(), r_ord[p_idx].end(), [p_idx] (const int& i, const int& j) {
            return mp(s_value[p_reqs[p_idx][i].fst],mp(p_reqs[p_idx][i].snd,i))>mp(s_value[p_reqs[p_idx][j].fst],mp(p_reqs[p_idx][j].snd,j));
        });
    }
    ...
    set<int> p_used;
    int p_ord_idx[2];
    forn(f,2) p_ord_idx[f]=0;
    int sqP = 10;
    
    while (min(p_ord_idx[0],p_ord_idx[1])<P) {
        
        SingleAnswer ans(-1);
        forn(f,2) {
            int L = sqP;
            for(int i = 0 ; i<L && i+p_ord_idx[f]<P ; i++) {
                int p_idx = p_ord[f][i+p_ord_idx[f]];
                if (p_used.count(p_idx)) {
                    swap( p_ord[f][i+p_ord_idx[f]],p_ord[f][p_ord_idx[f]]);
                    L++;
                    p_ord_idx[f]++;
                } else {
                    SingleAnswer curr = assign_pidx(p_idx);
                    if (curr.fail) {
                        swap( p_ord[f][i+p_ord_idx[f]],p_ord[f][p_ord_idx[f]]);
                        p_ord_idx[f]++;
                    } else if (  ans.p_idx ==-1 ||
        ...
        D(min(p_ord_idx[0],p_ord_idx[1]));
        if ( !ans.fail ) {
            // Updates anss, c_busy, c_skill, p_used
            int R =si(p_reqs[ans.p_idx]); 
            assert(si(ans.c_selecteds)==R);
            anss.pb(mp(ans.p_idx,ans.c_selecteds));
            forn(r_idx,R) {
                int c_idx = ans.c_selecteds[r_idx];
                c_busy[c_idx] = ans.t_end;
                
                int s_idx = p_reqs[ans.p_idx][r_idx].fst;
                int req_lvl = p_reqs[ans.p_idx][r_idx].snd; 
                auto it = c_skill[c_idx].find(s_idx);
                int c_lvl = it == c_skill[c_idx].end() ? 0 : it->snd;
                if (req_lvl >= c_lvl) {
                    c_skill[c_idx][s_idx]++;
            ...
            p_used.insert(ans.p_idx);
            D(ans.real_value);
            anss_value += ans.real_value;
        } else {
            forn(f,2) p_ord_idx[f]+=sqP;
            D("FAIL");
        }
